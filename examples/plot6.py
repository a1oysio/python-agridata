...
B) Prezzo medio annuo con banda tra minimo e massimo per anno.
...
def main():
    ...
    for col in ['grossProduction', 'area', 'yield']:
        ...
        z.loc[0, col] = 0  # primo valore a 0 senza warning
    ...
    # -------------------------------------------------------------------------
    # B) PREZZO MEDIO ANNUO con banda MIN–MAX per anno
    # -------------------------------------------------------------------------
    ...
    # Calcolo mean, min e max per anno
    annual_price = (
        df_prices
    ...
    # -------------------------------------------------------------------------
    # Unisci solo year, grossProduction, area (da annual) con price_mean
    combined = pd.merge(
        annual,
        annual_price[['year', 'price_mean']],
        on='year',
        how='inner'
    )
