...
@dataclass
class CerealProductionParams:
    """Allowed parameters for the cereal production endpoint."""
...
class CerealProductionQuery(BaseQuery):
    VALID_PARAMS = {f.name for f in fields(CerealProductionParams)}

    def __init__(self, **params):
        super().__init__(**params)
        self.params_obj = CerealProductionParams(**params)
